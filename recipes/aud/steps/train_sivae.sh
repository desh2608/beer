...
# This script trains an SIVAE-HMM. Note that
# the util scripts used are same as that for
# the VAE-HMM, only the configuration file
# has changed, since only the encoder becomes
# hierarchical for SIVAE.
...
fea=$fea_dir/${aud_sivae_hmm_fea_type}.npz
feastats=$fea_dir/${aud_sivae_hmm_fea_type}.stats.npz


if [ ! -f $mdl_dir/0.mdl ]; then
    echo "Building the SIVAE-HMM AUD model..."

    cat $aud_sivae_hmm_hmm_conf | sed s/{n_units}/$aud_sivae_hmm_n_units/g \
        > $mdl_dir/hmm.yml || exit 1

    # Creete the unigral LM of the acoustic units.
    python utils/lm-unigram-create.py \
        --concentration $aud_sivae_hmm_lm_concentration \
    ...
    # Create the VAE.
    vars="dim_in=$feadim,dim_out=$aud_sivae_hmm_nnet_width"
    python utils/nnet-create.py \
        --set "$vars" \
        $aud_sivae_hmm_encoder_conf \
        $mdl_dir/encoder.mdl || exit_msg "Failed to create the VAE encoder"

    vars="dim_in=$aud_sivae_hmm_latent_dim,width=$aud_sivae_hmm_nnet_width,dim_out=$feadim"
    python utils/nnet-create.py \
        --set "$vars" \
        $aud_sivae_hmm_decoder_conf \
        $mdl_dir/decoder.mdl || exit_msg "Failed to create the VAE decoder"

    vars="dim_in=$aud_sivae_hmm_latent_dim"
    python utils/nflow-create.py \
        --set "$vars" \
        $aud_sivae_hmm_nflow_conf \
        $mdl_dir/nflow.mdl || exit_msg "Failed to create the VAE norm. flow"

    python utils/vae-hmm-create.py \
        --encoder-cov-type $aud_sivae_hmm_encoder_cov_type \
        --decoder-cov-type $aud_sivae_hmm_decoder_cov_type \
        $feastats \
        $aud_sivae_hmm_nnet_width \
        $aud_sivae_hmm_latent_dim \
        $mdl_dir/encoder.mdl \
        $mdl_dir/nflow.mdl \
...
if [ ! -f $mdl_dir/final.mdl ];then
    ...
    while [ $((++iter)) -le $aud_sivae_hmm_train_iters ]; do
        echo "Iteration: $iter"

        if echo $aud_sivae_hmm_align_iters | grep -w $iter >/dev/null; then
            echo "Aligning data"
            ...
            utils/parallel/submit_parallel.sh \
                ...
                "$aud_sivae_hmm_align_parallel_opts" \
                "$aud_sivae_hmm_align_njobs" \
        ...
        if [ $iter -le $aud_sivae_hmm_train_warmup_iters ]; then
        ...
        cmd="python -u utils/vae-hmm-train-with-alignments.py \
                --verbose \
                --batch-size $aud_sivae_hmm_train_batch_size \
                --lrate $aud_sivae_hmm_train_lrate \
                --lrate-nnet $aud_sivae_hmm_train_nnet_lrate \
                --epochs $aud_sivae_hmm_train_epochs_per_iter \
                --nnet-optim-state $mdl_dir/nnet_optim_state.pkl \
                $kl_div_weight \
                $aud_sivae_hmm_train_opts \
                $mdl_dir/$((iter - 1)).mdl \
                $mdl_dir/alis.npz \
                $fea \
                $feastats \
        ...
        utils/parallel/submit_parallel.sh \
            ...
            "$aud_sivae_hmm_train_parallel_opts" \
            "$aud_hmm_train_njobs" \
            "$data_train_dir/uttids" \
            "$cmd" \
            $mdl_dir || exit 1
